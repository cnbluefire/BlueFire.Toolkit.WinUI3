...
namespace BlueFire.Toolkit.Sample.WinUI3.Views.ToolPages
{
    ...
    /// </summary>
    public sealed partial class LinearGradientBlurBackdropPage : Page
    {
        public LinearGradientBlurBackdropPage()
        {
            this.InitializeComponent();
        }

        public ToolModel ToolModel => ViewModelLocator.Instance.MainWindowViewModel.AllTools
            .First(c => c.Name == "LinearGradientBlurBackdrop").ToolModel;

        private void ClickWindowButton_Click(object sender, RoutedEventArgs e)
        {
            #region Block 1

            var window = new BackdropSampleWindow(nameof(LinearGradientBlurBackdrop));
            window.Activate();
